...
export default class Sqids {
  ...
  private toNumber(id: string, alphabet: string): number {
    ...
    return id
      .split("")
      .reduce((a, v) => a * chars.length + chars.indexOf(v), 0);
  }
...
export function encrypt(id: number): string {
  ...
  return `${PREFIX}${sqids.encode([id])}`;
}

export function decrypt(id: string): number {
  const sqids = new Sqids({
    alphabet: ALPHANUMERIC,
    minLength: 10,
  });
  if (id.startsWith(PREFIX)) {
    return sqids.decode(id.slice(PREFIX.length))[0];
  }
